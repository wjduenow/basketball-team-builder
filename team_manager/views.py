...
def index(request):
    ...
    best_tandem_point_differential = PlayerPlayerSummary.objects.filter(played__sum__gt=4).order_by("-point_differential")[:5]
    best_tandem_win_ratio = PlayerPlayerSummary.objects.filter(played__sum__gt=4).order_by("-win_loss")[:5]
